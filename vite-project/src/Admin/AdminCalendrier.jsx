...
export default function AdminCalendrier() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        setEvents(Array.isArray(data) ? [...data].sort(sortByDateAsc) : []);
      } catch {
  ...
  const resetForm = () => {
    setFormData({ dateEvent: "", heure: "", titre: "", statut: "Confirmé" });
    setEditingId(null);
    setError("");
    setMessage("");
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        setEvents((prev) => prev.map((ev) => (ev.id === editingId ? data : ev)).sort(sortByDateAsc));
      ...
      <div className="grid gap-4 lg:grid-cols-2 lg:gap-6">
        ...
        <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-4 md:p-5">
          ...
          <form className="space-y-3 text-xs md:text-sm" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-1">
              ...
              <input type="date" name="dateEvent" value={formData.dateEvent} onChange={handleChange} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-red-500" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-slate-400">Heure</label>
              <input type="time" name="heure" value={formData.heure} onChange={handleChange} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-red-500" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-slate-400">Titre / Match</label>
              <input type="text" name="titre" value={formData.titre} onChange={handleChange} placeholder="Match vs Angers, entraînement, soirée DOGZ..." className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-red-500" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-slate-400">Statut</label>
              <select name="statut" value={formData.statut} onChange={handleChange} className="bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-red-500">
        ...
        <section className="bg-slate-950/70 border border-slate-800 rounded-2xl p-4 md:p-5">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm md:text-base font-semibold">Événements à venir</h2>
              ...
              <div key={e.id} className="px-3 py-3 flex items-center justify-between">
                <div>
                  <p className="text-xs md:text-sm font-semibold">
                    ...
                    {e.heure && (<>• <span className="text-slate-300">{e.heure}</span></>)}
                  </p>
                  <p className="text-[11px] md:text-xs text-slate-400">{e.titre}</p>
                  <p className="text-[11px] text-slate-500">Statut : {e.statut}</p>
                </div>
                <div className="flex flex-col gap-2">
                  <button type="button" onClick={() => handleEdit(e)} className="text-[11px] text-slate-300 border border-slate-600 rounded-full px-3 py-1 hover:bg-slate-800">
                    Modifier
                  </button>
                  <button type="button" onClick={() => handleDelete(e.id)} className="text-[11px] text-red-300 border border-red-500 rounded-full px-3 py-1 hover:bg-red-950/40">
                    Supprimer
                  </button>
                </div>
              </div>
